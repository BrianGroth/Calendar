/*
  Calendar App — app.js (v3)
  - Fix: Weekday headers now align as top row across 7 columns (no nested sub-grid)
  - Feature: Scroll into the future up to 24 months (2 years)
*/
...
;(() => {
  ...
  function loadSettings(){
    try{ const j = localStorage.getItem(SETTINGS_KEY); if(!j) return { ...defaultSettings }; const p = JSON.parse(j); return { ...defaultSettings, ...p, holidays: { ...defaultSettings.holidays, ...(p.holidays||{}) } }; }catch{ return { ...defaultSettings }; }
  }
  ...
  const addMonths = (d,delta)=> { const x = new Date(d.getFullYear(), d.getMonth(), 1); x.setMonth(x.getMonth()+delta); return x; };
  ...
  const GH = {
    async loadMonth(settings, y, m){
      ...
      }catch(e){ consoleWarn('Failed to load month', e); return { events: [], sha: null }; }
    },
  ...
  async function getOverlay(key, url, monthDate){ if(!url) return []; const k=`${key}|${fmtMonthKey(monthDate)}`; if(OverlayCache.has(k)) return OverlayCache.get(k); try{ const raw=await fetchICS(url); const m=filterEventsToMonth(raw, monthDate); OverlayCache.set(k,m); return m; }catch(e){ consoleWarn('Overlay fetch failed '+key, e); return []; } }
  ...
  class CalendarApp{
    constructor(rootId='monthsContainer'){
      ...
      if(!this.root) throw new Error(`#${rootId} not found`);
...
this.ensureMonth(base);
    ...
    async renderInitial(){
      ...
      const next=new Date(now.getFullYear(), now.getMonth()+1, 1); if(next<=this.maxFuture) await this.ensureMonth(next);
    }
    ...
    async renderMonthSection(date){
      ...
      const toggles=document.createElement('div'); toggles.className='holiday-toggles'; ['us','uk','nl'].forEach(cc=>{ const b=document.createElement('button'); b.type='button'; b.className='holiday-toggle'; b.dataset.country=cc; b.setAttribute('aria-pressed','false'); b.textContent=cc.toUpperCase(); b.addEventListener('click', ()=> this.toggleHolidayOverlay(cc, date, wrap, b)); toggles.appendChild(b); }); header.appendChild(toggles);
    ...
    async toggleHolidayOverlay(country, date, section, btn){
      ...
      const map = { us:this.settings.holidays.usUrl, uk:this.settings.holidays.ukUrl, nl:this.settings.holidays.nlUrl };
      const url = country==='tripit'? this.settings.tripitIcalUrl : map[country];
      if(!url && country!=='tripit'){ announceStatus(`${country.toUpperCase()} holiday URL not configured in Settings.`); return; }
      const grid = section.querySelector('.month-grid'); if(!grid) return;
      const base = section.__events||[]; const overlay = !pressed? await getOverlay(country, url, date) : [];
      await this.renderGrid(date, grid, base.concat(overlay));
    }

    async renderGrid(date, grid, events){
      grid.innerHTML='';
      // 1) Header: add 7 weekday cells directly as items of the grid
      ...
      const firstWeekday = (first.getDay() + 6) % 7; // Monday-first (0..6)
      ...
      for(let i=0;i<totalCells;i++){
        ...
        events.filter(e=> e.date===iso).forEach(ev=> {
          const li=document.createElement('li'); li.className='event-item' + (ev.__overlay? ' overlay':'');
          const btn=document.createElement('button'); btn.type='button'; btn.className='event-btn'; const prefix= ev.startTime? `${ev.startTime} `:''; btn.textContent=`${prefix}${ev.title}`; btn.title=ev.notes||''; btn.addEventListener('click', ()=>{ if(ev.__overlay && ev.url){ window.open(ev.url, '_blank'); } else { this.openEventModal(ev); } }); li.appendChild(btn);
          if(ev.url && ev.__overlay){ const a=document.createElement('a'); a.href=ev.url; a.target='_blank'; a.rel='noopener noreferrer'; a.className='event-link'; a.textContent='↗'; li.appendChild(a); }
          list.appendChild(li);
        });
    ...
    // Modal
    bindModal(){
      this.modal=document.getElementById('eventModal'); if(!this.modal){ consoleWarn('No #eventModal found'); return; }
    ...
    async onSaveEvent(){
      ...
      try{ const y=Number(key.slice(0,4)), m=Number(key.slice(5,7)); const res=await GH.saveMonth(this.settings, y, m, base, section.dataset.sha||null); section.dataset.sha = res?.content?.sha || res?.sha || section.dataset.sha || ''; section.__events=base; announceStatus(`Saved to data/${key}.json`);
        const grid=section.querySelector('.month-grid'); const active=Array.from(section.querySelectorAll('.holiday-toggle[aria-pressed="true"]')).map(b=>b.dataset.country); let combo=base.slice(); for(const cc of active){ const map={us:this.settings.holidays.usUrl, uk:this.settings.holidays.ukUrl, nl:this.settings.holidays.nlUrl}; const url= cc==='tripit'? this.settings.tripitIcalUrl : map[cc]; const evts= await getOverlay(cc, url, parseISODate(key+'-01')); combo=combo.concat(evts); } await this.renderGrid(parseISODate(key+'-01'), grid, combo);
      }catch(e){ consoleWarn('Save failed', e); announceStatus('Save failed. Check token permissions and network.'); }
      this.closeModal();
    }
    ...
    async onDeleteEvent(){
      ...
      try{ const y=Number(key.slice(0,4)), m=Number(key.slice(5,7)); const res=await GH.saveMonth(this.settings, y, m, base, section.dataset.sha||null); section.dataset.sha=res?.content?.sha || res?.sha || section.dataset.sha || ''; section.__events=base; announceStatus(`Deleted. Saved to data/${key}.json`); const grid=section.querySelector('.month-grid'); await this.renderGrid(parseISODate(key+'-01'), grid, base); }catch(e){ consoleWarn('Delete failed', e); announceStatus('Delete failed.'); }
      this.closeModal();
    }
  }

  document.addEventListener('DOMContentLoaded', ()=>{ try{ new CalendarApp('monthsContainer'); announceStatus('Calendar ready.'); }catch(e){ consoleWarn('Calendar init failed', e); announceStatus('Calendar failed to initialize. Check console.'); } });
})();
